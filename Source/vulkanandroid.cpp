...
{        
    __android_log_print(ANDROID_LOG_INFO, "LunaInfo", "Loading libvulkan.so...\n");

    // Load vulkan library
    libVulkan = dlopen("libvulkan.so", RTLD_NOW | RTLD_LOCAL);
    if (!libVulkan)
    {
        __android_log_print(ANDROID_LOG_INFO, "LunaInfo", "Could not load vulkan library : %s!\n", dlerror());
        return false;
    }
...
{
    __android_log_print(ANDROID_LOG_INFO, "LunaInfo", "Loading instance based function pointers...\n");
