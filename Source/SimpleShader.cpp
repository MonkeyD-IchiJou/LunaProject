...
namespace luna
{
	...
	void SimpleShader::SetDescriptors(const VulkanImageBufferObject * finalimage)
	{
	...
	void SimpleShader::Init(const VkRenderPass & renderpass, uint32_t subpassindex)
	{
		...
		if (m_Pipeline == VK_NULL_HANDLE)
		{
			...
			VkPipelineShaderStageCreateInfo fraginfo = CreateShaders_(getAssetPath() + "Shaders/simpleshader_frag.spv");
	...
	void SimpleShader::SetUpFixedPipeline_(FixedPipelineCreationTool & fixedpipeline)
	{
		// no depth test
