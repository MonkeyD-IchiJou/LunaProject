...
namespace luna
{
		...
		const UBO* pointlights_ubo)
	{
		...
		layoutinfo[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		layoutinfo[1].typeflags = 0; // a buffer
		...
		poolSizes[0].descriptorCount = 1; // 1 input attachments
		...
		firstdescriptorset[1].bufferinfo = uboinfo;
