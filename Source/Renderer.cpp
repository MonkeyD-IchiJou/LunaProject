...
namespace luna
{
	...
	void Renderer::InitVulkanInstance_()
	{
	...
	void Renderer::SetUpLogicalDevice_()
	{
	...
	void Renderer::InitLogicalDevice_()
	{
		float queue_priorities[]{ 1.0f };
		VkDeviceQueueCreateInfo device_queue_create_info{};
		device_queue_create_info.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
		device_queue_create_info.queueFamilyIndex = m_queuefamily_index.graphicsFamily; // will create one queue for this logical device based on the index
	...
	const uint32_t Renderer::findMemoryType(const uint32_t & typeFilter, const VkMemoryPropertyFlags & properties) const
	{
	...
	const VkFormat Renderer::findSupportedFormat(const std::vector<VkFormat>& candidates, const VkImageTiling & tiling, const VkFormatFeatureFlags & features) const
	{
	...
	void Renderer::PhysicalDevice::Init(const VkInstance & instance)
	{
	...
	QueueFamilyIndices Renderer::PhysicalDevice::findQueueFamilies(const uint32_t & whichOne)
	{
