...
namespace luna
{
	...
	void SubPassShader::Bind(const VkCommandBuffer & commandbuffer)
	{
	...
	void SubPassShader::SetDescriptors(const VulkanImageBufferObject * finalimage)
	{
		...
		imagedescriptor.type = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT; // take in input attachment
	...
	void SubPassShader::Init(const VkRenderPass & renderpass)
	{
		...
		if (m_graphicPipeline == VK_NULL_HANDLE)
		{
			...
			VkPipelineShaderStageCreateInfo fraginfo = CreateShaders_(getAssetPath() + "Shaders/subpass_frag.spv");
	...
	void SubPassShader::SetUpFixedPipeline_(FixedPipelineCreationTool & fixedpipeline)
	{
		// no depth test
	...
	void SubPassShader::CreatePipelineLayout_()
	{
